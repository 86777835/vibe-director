...
Skill 检测到目录是空的（无 `wiki/index.md`），按 **3 步**带你过：

#### 4.1 你想怎么开始？（模式）

Agent 弹按钮卡片让你选（不需要打字输 A/B/C）：

- **原创剧本** — 从零开始（题材 → 创作方案 → 角色 → 剧本）
- **已有剧本要可视化** — 我有剧本，只想出故事板/视频（Mode B）
- **小说改编** — 给我一本小说，改成剧（Mode C）
- **导入既有素材** — 已有完整短剧（剧本+角色+世界观），一次性入库（Mode D）

> Mode E（方向性重构）不在这一步——它需要已有 wiki，从 active wiki 触发。

也可以**直接说自然语言**——"我想做新剧" / "我有剧本要出图" / "改这本小说" / "我有完整素材"，agent 自动判断。

#### 4.2 做到哪一步？（范围）

仅当模式选了"原创/改编/导入"才问这步（"已有剧本要可视化"明显已经需要故事板）：

- **只写剧本** — 纯文字，无需 API
- **剧本 + 故事板** — 需要图片生成 API
- **剧本 + 故事板 + 视频** — 需要全套 API（最贵）

#### 4.3 API 配置（如选 +故事板 或 +视频）

会创建 `wiki/api-config.md`，引导填入：
